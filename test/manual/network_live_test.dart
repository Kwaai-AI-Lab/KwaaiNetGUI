...
void main() {
  test('network op serves live swarm state', () async {
    ...
    expect(first.selfStatus.usingRelay, first.selfStatus.relayAddrs.isNotEmpty);
    ...
    for (final r in first.routing.where((r) => r.connected)) {
      expect(
        ...
        isTrue,
        reason: 'routing peer ${r.peerId} claims connected but is not in the '
    ...
    expect(
      updates.length,
      lessThanOrEqualTo(5),
      reason: 'more updates than sample ticks means suppression is not working',
    );
