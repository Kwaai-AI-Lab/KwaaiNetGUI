...
void main() {
  test(
    'the feed keeps flowing past the heartbeat',
    () async {
      ...
      expect(
        ...
        reason:
            'only one update in 75s: the heartbeat never fired, so the '
  ...
  test(
    'repeated subscribe/cancel does not wedge the session',
    () async {
      final client = KwaaiRpcClient();
      addTearDown(client.close);
      ...
      for (var i = 0; i < 10; i++) {
        ...
        final sub = client
            .networkStream(intervalSecs: 5)
            .listen((u) => got ??= u, onError: (Object e) => err = e);
        ...
        expect(
          ...
          isNotNull,
          reason:
              'cycle $i produced no update — the session stopped serving '
